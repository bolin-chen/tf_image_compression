...
def crop_image_input_patches(image, patch_size):
  ...
  for i in range(height_patch_num):
    for j in range(width_patch_num):
      patch = padded_image[i * patch_size : (i + 1) * patch_size, j * patch_size : (j + 1) * patch_size, :]
      image_patches.append(patch)

      # scipy.misc.imsave('other/temp_patch/{}_{}.png'.format(i, j), patch)
...
def concat_patches(patches, height, width, patch_size):
  if height % patch_size != 0:
