...
def run_rmbe_model(patch_list):
  ...
  with sess.as_default():
    with g2.as_default():
      ...
      output_op = rmbe_model.model(patch_batch, quan_scale)

      params_file = 'rmbe_params/params'
...
def rmbe_height(image, height, width, offset):
  ...
  for i in range(height_patch_num):
    for j in range(width_patch_num):
      patch = image[i * patch_size : (i + 1) * patch_size, offset + j * patch_size : (j + 1) * patch_size, :]
  ...
  for i in range(height_patch_num):
    for j in range(width_patch_num):
      new_image[i * patch_size : (i + 1) * patch_size, offset + j * patch_size : (j + 1) * patch_size, :] = new_patch_list[i * width_patch_num + j]
...
def rmbe_width(image, height, width, offset):
  ...
  for i in range(height_patch_num):
    for j in range(width_patch_num):
      patch = image[offset + i * patch_size : (i + 1) * patch_size, j * patch_size : (j + 1) * patch_size, :]
      patch_list.append(patch)
  ...
  for i in range(height_patch_num):
    for j in range(width_patch_num):
      new_image[offset + i * patch_size : (i + 1) * patch_size, j * patch_size : (j + 1) * patch_size, :] = new_patch_list[i * width_patch_num + j]
