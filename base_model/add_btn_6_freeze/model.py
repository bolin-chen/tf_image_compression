...
def encoder(input, patch_size, quan_scale, bottleneck_channel):
...
def optimize(loss, boundaries, lr_values, var_list=None):
...
def train(sess, args):
  ...
  output = encoder(data_batch, patch_size, quan_scale, bottleneck_channel)
  ...
  if fine_tune != 'off':
    if fine_tune == 'btnk':
      not_restore = ['encode_3', 'decode_3']

    if fine_tune == 'input':
      if patch_size == 256:
        not_restore = ['encode_0', 'encode_1', 'decode_1', 'decode_0']
      elif patch_size == 512:
        not_restore = ['encode_m1', 'encode_0', 'decode_0', 'decode_m1']

    var_list = [var for var in tf.trainable_variables() if var.name.split('/')[0] in not_restore]
  else:
    var_list = None


  train_op, global_step_op, learning_rate_op = optimize(loss_op, boundaries, lr_values, var_list)
  ...
  if fine_tune != 'off':
    ...
    restore_var_list = [var for var in tf.trainable_variables() if var.name.split('/')[0] not in not_restore]

    saver = tf.train.Saver(restore_var_list)
    saver.restore(sess, model_param_path)
    print('Load fine tune params complete')
  ...
  if args.debug_mode == 'on':
    ...
    # [print(var.name) for var in tf.trainable_variables() if var.name.split('/')[0] not in not_restore]
